...
def get_parser():
    parser = argparse.ArgumentParser(
        description='SkateFormer: Skeletal-Temporal Transformer for Human Action Recognition')
    parser.add_argument('--work-dir', default=None, help='the work folder for storing results')
    parser.add_argument('--model_saved_name', default='')

    # === config =====================================================================
    parser.add_argument('--config', default='./config/SkateFormer_j_NEW_head.yaml',
                        help='path to the configuration file')
    # =================================================================================

    parser.add_argument('--weights', default="contrastive_model_epoch17.pth",
    ...
    parser.add_argument('--min-lr', type=float, default=0.01, help='minimum learning rate')
    parser.add_argument('--warmup-lr', type=float, default=0.01, help='warmup learning rate')
...
class Processor():
    ...
    def load_model(self):
        ...
        if self.arg.weights:
            ...
            # 处理权重：移除 'module.' 与 'backbone.' 前缀，并过滤掉 head 层的权重（因仅训练 head 层）
            new_weights = OrderedDict()
            for k, v in weights.items():
                key = k.split('module.')[-1]
                if key.startswith("backbone."):
                    key = key[len("backbone."):]
                # 如果是 head 层权重，则跳过，head 层将随机初始化
                if key.startswith("head."):
                    continue
                new_weights[key] = v.cuda(output_device)
            # 使用 strict=False 允许 head 层缺失
            self.model.load_state_dict(new_weights, strict=False)

        # 冻结（freeze）除最后一层 head 外的所有参数，只训练 head 层
        for name, param in self.model.named_parameters():
            if "head" not in name:  # 请根据实际情况修改最后一层的命名，如 "fc" 或 "classifier"
                param.requires_grad = False
            else:
                param.requires_grad = True  # 确保 head 层可训练

    def load_optimizer(self):
        # 仅传入 requires_grad=True 的参数，即只训练 head 层
