...
def augment2(x, amp1=0.01, amp2=0.01):
    ...
    """
    B, C, T, V, M = x.shape
    # 为每个样本的每一帧生成初始为单位矩阵的变换矩阵，形状为 [B, T, 2, 2]
    A = torch.eye(2, device=x.device).unsqueeze(0).unsqueeze(0).repeat(B, T, 1, 1)
    # 为每一帧加入随机扰动，amp1 控制扰动幅度
    A = A + torch.randn(B, T, 2, 2, device=x.device) * amp1

    # 调整 x 的维度，使 xy 坐标维度放到最后：从 [B, C, T, V, M] -> [B, T, V, M, C]
    x_perm = x.permute(0, 2, 3, 4, 1)
    # 使用 torch.einsum 对每一帧的 xy 坐标进行线性变换
    x_transformed = torch.einsum('btij,btvmi->btvmi', A, x_perm)
    # 添加随机噪声，amp2 控制噪声幅度
    x_transformed = x_transformed + torch.randn_like(x_transformed) * amp2
    # 将维度还原回原始顺序 [B, C, T, V, M]
...
def get_parser():
    ...
    parser.add_argument('--config', default='./config/SkateFormer_6w_s_j_NEW_c.yaml', help='path to the configuration file')
    # ===config==========================================================================================================================================
    ...
    # parser.add_argument('--weights', default="./xxx_62_30.pt", help='the weights for network initialization')  # 60.67%
    # ===config==========================================================================================================================================
...
class Processor():
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, index_t, label, index) in enumerate(process):
            ...
            output_forward_head, output_projection_head, output_coarse_head = self.model(big_batch, big_batch_index_t)
            ...
            output_coarse = output_coarse_head[:B]  # Original data output, shape [B, num_classes]
            ...
            # loss = 0.3 * self.loss(output, label) + 0.6 * self.contrast_loss(z1, z2) + 0.3 * self.loss(output_coarse, coarse_labels)
    ...
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None):
        ...
        for ln in loader_name:
            ...
            for batch_idx, (data, index_t, label, index) in enumerate(process):
                ...
                label_list_coarse.append(coarse_labels)
                with torch.no_grad():
                    data = data.float().cuda(self.output_device)
                    index_t = index_t.float().cuda(self.output_device)
                    label = label.long().cuda(self.output_device)
                    coarse_labels = coarse_labels.long().cuda(self.output_device)
                    output, _, output_coarse = self.model(data, index_t)

                    loss = self.loss(output, label)
                    loss_coarse = self.loss(output_coarse, coarse_labels)

                    score_frag.append(output.data.cpu().numpy())
                    score_frag_coarse.append(output_coarse.data.cpu().numpy())

                    loss_value.append(loss.data.item())
                    loss_value_coarse.append(loss_coarse.data.item())

                    _, predict_label = torch.max(output.data, 1)
                    _, predict_label_coarse = torch.max(output_coarse.data, 1)
                    pred_list.append(predict_label.data.cpu().numpy())
                    pred_list_coarse.append(predict_label_coarse.data.cpu().numpy())
                    step += 1
            ...
            score_coarse = np.concatenate(score_frag_coarse)
            loss = np.mean(loss_value)
            loss_coarse = np.mean(loss_value_coarse)
            if 'ma52' in self.arg.feeder:
                self.data_loader[ln].dataset.sample_name = np.arange(len(score))
            accuracy = self.data_loader[ln].dataset.top_k(score, 1)
            accuracy_coarse = self.data_loader[ln].dataset.top_k(score_coarse, 1, True)
            ...
            if self.arg.phase == 'train':
                ...
                self.val_writer.add_scalar('loss_coarse', loss_coarse, self.global_step)
                self.val_writer.add_scalar('acc_coarse', accuracy_coarse, self.global_step)

            score_dict = dict(zip(self.data_loader[ln].dataset.sample_name, score))
            self.print_log('\tMean {} loss_fine of {} batches: {}.'.format(ln, len(self.data_loader[ln]), np.mean(loss_value)))
            self.print_log('\tMean {} loss_coarse of {} batches: {}.'.format(ln, len(self.data_loader[ln]), np.mean(loss_value_coarse)))
            for k in self.arg.show_topk:
                self.print_log('\tTop{}_fine: {:.2f}%'.format(k, 100 * self.data_loader[ln].dataset.top_k(score, k)))
            for k in self.arg.show_topk:
                self.print_log('\tTop{}_coarse: {:.2f}%'.format(k, 100 * self.data_loader[ln].dataset.top_k(score_coarse, k, True)))

            # pred_label = np.argmax(score, axis=1)
            # macro_f1 = f1_score(label, pred_label, average='macro')
            # micro_f1 = f1_score(label, pred_label, average='micro')
            # self.print_log(f"F1 Macro: {macro_f1 * 100:.2f}%")
            # self.print_log(f"F1 Micro: {micro_f1 * 100:.2f}%")
