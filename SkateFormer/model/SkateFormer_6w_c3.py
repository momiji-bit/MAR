...
class SkateFormerBlock(nn.Module):
    def __init__(self, in_channels, num_points=50, kernel_size=7, num_heads=32,
        ...
        self.alpha_attn = nn.Parameter(torch.ones(len(self.partition_function)))  # 4 个 Attention
        self.alpha_GConv = nn.Parameter(torch.ones(1))
        self.alpha_TConv = nn.Parameter(torch.ones(1))

    def forward(self, input):
        # self.alpha_attn.data[2].fill_(2.0)
        # self.alpha_GConv.data.fill_(1.0)
        # self.alpha_TConv.data.fill_(2.0)
        ...
        for i in range(self.gconv.shape[0]):
            ...
            y_gconv.append(z)
        y.append(torch.cat(y_gconv, dim=1) * self.alpha_GConv)  # N C T V
        ...
        for i in range(len(self.partition_function)):
            ...
            att_out = self.reverse_function[i](self.attention[i](input_partitioned), (T, V), self.partition_size[i])
            y.append(self.alpha_attn[i] * att_out)
...
class SkateFormer(nn.Module):
    def __init__(self, in_channels=3, depths=(2, 2, 2, 2), channels=(96, 192, 192, 192), num_classes=60,
        ...
        #     # 先通过1×1卷积降低通道维度（也可视情况设置为中间维度）
        #     nn.Conv2d(in_channels, embed_dim // 2, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0), bias=False),
        #     nn.BatchNorm2d(embed_dim // 2),
        #     nn.ReLU(inplace=True),
        #     # 沿时间方向进行深度可分离卷积，捕捉局部动态信息
        #     nn.Conv2d(embed_dim // 2, embed_dim // 2, kernel_size=(3, 1), stride=(1, 1), padding=(1, 0), groups=embed_dim // 2, bias=False),
        #     nn.BatchNorm2d(embed_dim // 2),
        #     nn.ReLU(inplace=True),
        #     # 1×1卷积将通道数提升到目标维度
        #     nn.Conv2d(embed_dim // 2, embed_dim, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0), bias=False),
        #     nn.BatchNorm2d(embed_dim),
        #     nn.ReLU(inplace=True)
        # )
