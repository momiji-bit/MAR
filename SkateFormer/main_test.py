...
class LabelSmoothingCrossEntropy(nn.Module):
    ...
    def forward(self, x, target):
        """
        x: [batch_size, num_classes] -> 模型输出的 logits
        target: [batch_size] -> 真实标签, 每个元素都是 [0, num_classes-1] 之间的索引
        """
        logprobs = F.log_softmax(x, dim=-1)  # 先对 logits 做 log-softmax

        # gather可在维度上抽取指定索引的值，如 logprobs.gather(dim=-1, index=target)
        # 注意 target 为 [N]，需要 view(-1, 1) 或 unsqueeze(1) 以适应 gather 的输入
        nll_loss = -logprobs.gather(dim=-1, index=target.unsqueeze(1))
        nll_loss = nll_loss.squeeze(1)

        # smooth_loss：对所有类别的平均
        smooth_loss = -logprobs.mean(dim=-1)

        # 最终的加权组合：confidence * nll_loss + smoothing * smooth_loss
...
class Processor():
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, index_t, label, index) in enumerate(process):
            ...
            self.global_step += 1
            with torch.no_grad():
                data = data.float().cuda(self.output_device)  # 64,3,64,28,1 BS,C,T,V,1
        ...
        for ln in loader_name:
            ...
            for batch_idx, (data, index_t, label, index) in enumerate(process):
                ...
                with torch.no_grad():
                    ...
                    output, _, _ = self.model(data, index_t)
