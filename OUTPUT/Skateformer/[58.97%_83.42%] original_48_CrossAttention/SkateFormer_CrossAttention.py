...
class CrossAttention(nn.Module):
    ...
    一个简单的多头交叉注意力示例：
      - Q 来自特征 x_q
      - K、V 来自特征 x_kv
      形状约定：x_q, x_kv 的输入形状均为 [B, N, C]
    """
    def __init__(self, in_channels, num_heads=8, dropout=0.):
        super().__init__()
        self.in_channels = in_channels
        self.num_heads = num_heads
    ...
    def forward(self, x_q, x_kv):
        ...
        k = self.k_proj(x_kv).view(B, Nk, self.num_heads, self.head_dim)
        v = self.v_proj(x_kv).view(B, Nk, self.num_heads, self.head_dim)

        # 2) 调整维度: [B, num_heads, N, head_dim]
        q = q.permute(0, 2, 1, 3)  # [B, num_heads, Nq, head_dim]
        k = k.permute(0, 2, 1, 3)  # [B, num_heads, Nk, head_dim]
        v = v.permute(0, 2, 1, 3)  # [B, num_heads, Nk, head_dim]
        ...
        out = out.permute(0, 2, 1, 3).reshape(B, Nq, C)  # 还原形状 [B, Nq, C]
...
class SkateFormerBlock(nn.Module):
    def __init__(self, in_channels, num_points=50, kernel_size=7, num_heads=32,
        ...
        # 增加一个 CrossAttention，用于演示
        # 这里 num_heads 也可以不同，看需求自行设定
        # self.cross_attn = CrossAttention(in_channels // 2, num_heads=num_heads // 2, dropout=attn_drop)
        self.cross_attn = CrossAttention(in_channels // 2, num_heads=4, dropout=attn_drop)
    ...
    def forward(self, input):
        ...
        for i in range(self.gconv.shape[0]):
            ...
            y_gconv.append(z)
        gconv_output = torch.cat(y_gconv, dim=1)  # N C T V  32,24,64,48

        # T-Conv
        tconv_output = self.tconv(split_f_conv[1])  # 32,24,64,48
        ...
        for i in range(len(self.partition_function)):
            ...
            y_msa.append(self.reverse_function[i](self.attention[i](input_partitioned), (T, V), self.partition_size[i]))
        msa_output = torch.cat(y_msa, dim=1)  # 32,48,64,48

        # 把 tconv 与 gconv 也先合并一下 (可按需求)
        tgconv_output = torch.cat([tconv_output, gconv_output], dim=1)  # => [B, C_tg, T, V]

        # 1) 调整形状到 [B, N, C]，其中 N = T*V
        B_, C_tg, T_, V_ = tgconv_output.shape
        B_, C_msa, T_, V_ = msa_output.shape

        # Flatten (T, V) -> N
        tgconv_flat = tgconv_output.reshape(B_, C_tg, T_ * V_).permute(0, 2, 1)  # => [B, N, C_tg]
        msa_flat = msa_output.reshape(B_, C_msa, T_ * V_).permute(0, 2, 1)  # => [B, N, C_msa]

        # 2) 根据 CrossAttention 的维度需求，若 C_tg != in_channels//2，需要额外线性映射
        #   下面假设恰好匹配 (比如 in_channels//2 = C_tg)，否则你需加个线性层把通道映射到统一维度。
        #   此处示例：令 tgconv_flat 做 Query， msa_flat 做 Key/Value
        cross_attn_out = self.cross_attn(tgconv_flat, msa_flat)  # => [B, N, C_tg]

        # 3) 还原回 [B, C_tg, T, V]
        cross_attn_out = cross_attn_out.permute(0, 2, 1).view(B_, C_tg, T_, V_)

        # 4) 可选择将 cross_attn_out 与原分支结果相加或拼接
        #   例如与 tgconv_output 做残差相加
        cross_attn_out = cross_attn_out + tgconv_output

        # 也可再与 msa_output 拼接
        # 视需求而定，这里示例简单地在通道维度拼接
        fusion = torch.cat([cross_attn_out, msa_output], dim=1)  # => [B, C_tg + C_m, T, V]

        output = self.proj(fusion.permute(0, 2, 3, 1).contiguous())  # => [B, T, V, in_channels]
        output = self.proj_drop(output)
        output = skip + self.drop_path(output)

        # Feed Forward
        output = output + self.drop_path(self.mlp(self.norm_2(output)))
        output = output.permute(0, 3, 1, 2).contiguous()  # => [B, C, T, V]
        return output
